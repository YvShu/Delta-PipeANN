...
for _, config in enumerate(group_config):
    ...
    # 使用正则表达式提取所有步骤数和对应值
    pattern =  r'search current time:\s*(\d+).*?\n\s*50\s+[\d.]+\s+[\d.]+\s+[\d.]+\s+[\d.]+\s+[\d.]+\s+[\d.]+\s+[\d.]+\s+([\d.]+)'
...
ax.set_ylabel("Recall@10", fontsize=15)
plt.title(f'Recall@10 (L: 50)', fontsize=17, fontweight='bold', pad=5)
...
# 保存为高质量科研图像
plt.savefig(f'/mnt/hgfs/DataSet/experiment_results/Recall@10-L50.svg', dpi=150, bbox_inches='tight')
plt.show()
