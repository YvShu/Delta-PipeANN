...
for _, config in enumerate(group_config):
    ...
    # 使用正则表达式提取所有步骤数和对应值
    pattern = r'\[direct_insert\.cpp:253:INFO\]\s+Processed\s+\d+\s+tasks,\s+throughput:\s+([\d.]+)\s+tasks/sec\.'
...
plt.title(f'Throughput', fontsize=17, fontweight='bold', pad=5)
...
ax.legend(loc='lower right', fontsize=10, frameon=True, framealpha=0.9, bbox_to_anchor=(1.15, 0.0))
...
# 保存为高质量科研图像
plt.savefig(f'/mnt/hgfs/DataSet/experiment_results/Throughput.svg', dpi=150, bbox_inches='tight')
plt.show()
