...
namespace pipeann {
  ...
  void SSDIndex<T, TagT>::merge_deletes_lvq(const std::string &in_path_prefix, const std::string &out_path_prefix,
...
#pragma omp parallel for num_threads(populate_nthreads)
      for (uint64_t loc = loc_st; loc < loc_ed; ++loc) {
        ...
        LVQDiskNode<T> node = lvqnode_from_page(page_rbuf, loc);
...
#pragma omp parallel for num_threads(nthreads)
      for (uint64_t loc = loc_st; loc < loc_ed; ++loc) {
        ...
        if (nhood.size() > this->params.R) {
          ...
          std::vector<T> point_tmp(meta_.data_dim);
          for (size_t d = 0; d < meta_.data_dim; ++d) {
            uint8_t q_val = node.coords[d];
            point_tmp[d] = (static_cast<T>(q_val) * node.step) + node.minval;
          }
          coord_pool.push_back(std::move(point_tmp));
          node_coords_cache[id] = coord_pool.back().data();

          char *buf = nullptr;
          pipeann::alloc_aligned((void **)&buf, size_per_io, SECTOR_LEN);
          for (uint32_t k = 0; k < nhood.size(); k++) {
            if (node_coords_cache.find(nhood[k]) != node_coords_cache.end()) {
              continue;
            }
            uint64_t page_id = id2page(nhood[k]);
            ...
            for (unsigned j = 0; j < meta_.nnodes_per_sector; ++j) {
              ...
              if (nhood_set.find(cur_id) != nhood_set.end() && 
                  node_coords_cache.find(cur_id) == node_coords_cache.end()) {
          ...
          for (uint32_t j = 0; j < nhood.size(); ++j) {
            if (node_coords_cache.find(nhood[j]) == node_coords_cache.end()) {
              LOG(ERROR) << nhood[j] << " not found in node_coords_cache";
              exit(-1);
            }
            dists[j] = this->dist_cmp->compare(node_coords_cache[id],
                                               node_coords_cache[nhood[j]], 
          ...
          pipeann::prune_neighbors(pool, nhood, params, metric, [&](uint32_t a, uint32_t b) {
            if (node_coords_cache.find(a) == node_coords_cache.end()) {
              LOG(ERROR) << a << " not found in node_coords_cache";
              exit(-1);
            }
            if (node_coords_cache.find(b) == node_coords_cache.end()) {
              LOG(ERROR) << b << " not found in node_coords_cache";
              exit(-1);
            }
            float dist;
            dist = this->dist_cmp->compare(node_coords_cache[a],
                                           node_coords_cache[b], 
                                           (unsigned) aligned_dim);
            return dist;
          });
        ...
        LVQDiskNode<T> w_node = lvqnode_from_page(page_wbuf, new_id);

        memcpy(w_node.coords, node.coords, meta_.data_dim * sizeof(uint8_t));
        w_node.nnbrs = nhood.size();
        *(w_node.nbrs - 1) = w_node.nnbrs;
        w_node.minval = (float) node.minval;
        w_node.step = (float) node.step;
