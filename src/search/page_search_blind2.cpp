...
namespace pipeann {
  ...
  size_t SSDIndex<T, TagT>::page_search_blind2(const T *query1, const uint64_t k_search, const uint32_t mem_L,
    ...
    auto compute_exact_dists_and_push = [&](const LVQDiskNode<T> &node, const unsigned id) -> float {
    ...
    auto compute_and_push_nbrs = [&](LVQDiskNode<T> &node, unsigned &nk) {
      ...
      if (nbors_cand_size) {
        ...
        for (unsigned m = 0; m < nbors_cand_size; ++m) {
          ...
          const float nbor_dist = cur_expanded_dist;
          if (stats != nullptr) {
            stats->n_cmps++;
          }
    ...
    while (k < cur_list_size) {
      ...
      for (size_t i = 0; i < last_io_snapshot.size(); ++i) {
        ...
        std::vector<std::pair<float, LVQDiskNode<T>>> vis_cand;  // <dist, node>
