...
namespace pipeann {
  ...
  void SSDIndex<T, TagT>::do_beam_search_blind1(const T *query1, uint32_t mem_L, uint32_t l_search, const uint32_t beam_width,
    ...
    if (mem_L) {
      ...
      retset[cur_list_size].distance = std::numeric_limits<float>::max();
    ...
    while (!read_queue.empty()) {
      ...
      //   marker++;
      // }
      while (!read_queue.empty() && num_seen < beam_width) {
        ...
        if (r < l_search) {
          for (unsigned m = 0; m < node.nnbrs; ++m) {
            ...
            if (visited.find(nbor_id) == visited.end()) {
              ...
              read_queue.push({nbor_id, cur_expanded_dist});
            }
          }
        }
  ...
  template<typename T, typename TagT>
  size_t SSDIndex<T, TagT>::beam_search_blind1(const T *query, const uint64_t k_search, const uint32_t mem_L,
    ...
    std::vector<Neighbor> expanded_nodes_info;
    this->do_beam_search_blind1(query, mem_L, (uint32_t) l_search, (uint32_t) beam_width, expanded_nodes_info, nullptr,
