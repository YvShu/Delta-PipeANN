 ...
 * @LastEditTime: 2026-04-28 00:08:03
 * @LastEditors: Guyue
 * @FilePath: /Delta-PipeANN/src/search/ssd_search.cpp
 */
...
namespace pipeann {
  ...
  void SSDIndex<T, TagT>::do_ssd_search(const T *query1, uint32_t mem_L, uint32_t l_search, const uint32_t beam_width,
  ...
  template<typename T, typename TagT>
  size_t SSDIndex<T, TagT>::ssd_search(const T *query, const uint64_t k_search, const uint32_t mem_L,
    ...
    std::vector<Neighbor> expanded_nodes_info;
    this->do_ssd_search(query, mem_L, (uint32_t) l_search, (uint32_t) beam_width, expanded_nodes_info, nullptr,
